...
@Component
@Slf4j(topic = "API")
public class GetBlockHeaderByLimitNextServlet extends HttpServlet {

  @Autowired
  private Wallet wallet;
  private static final long BLOCK_LIMIT_NUM = 500;
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      if (endNum > 0 && endNum > startNum && endNum - startNum <= BLOCK_LIMIT_NUM) {
        ...
        if (reply != null) {
          response.getWriter().println(Util.printBlockListWithNoTx(reply, visible));
          return;
        }
      }
